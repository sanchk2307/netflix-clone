...
export class FirebaseService {
  ...
  async seedData() {
    ...
    try {
      const result = await this.addDataArray(seriesData, 'series');
      console.log(result);
    } catch (error) {
    ...
    try {
      const result = await this.addDataArray(filmsData, 'films');
      console.log(result);
    } catch (error) {
      console.error('Error seeding data:', error);
    }
  }
  async addDataArray(dataArray: any[], collectionName: string) {
    const collectionRef = collection(this.firestore, collectionName);
    const promises = dataArray.map((data) => addDoc(collectionRef, data));
  ...
  useContent(target: string) {
    const colRef = collection(this.firestore, target);
